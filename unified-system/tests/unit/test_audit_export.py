...
def test_export_results_handles_mixed_workflow_collection() -> None:
    ...
    patch = PatchExtractionResult(
        ...
        roi_id="roi-1",
        patch_id="patch-1",
        warnings=[AuditWarning(code="out_of_bounds", message="Patch exceeded image bounds")],
    )
    ...
    assert exported["results"][1]["transferred_roi"]["bbox"] == [1, 2, 3, 4]
...
def test_export_plain_mapping_and_custom_to_dict_result() -> None:
    ...
    custom_export = export_result(
        CustomResult({"status": ResultStatus.SUCCESS, "payload": {"axes": ("ml", "si")}})
    )

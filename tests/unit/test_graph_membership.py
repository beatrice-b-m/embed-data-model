...
from embed_data_model import DatasetGraph, Exam, Finding, MammogramImage, Patient, RegionOfInterest
...
def test_explicit_owner_clear_detaches_and_foreign_assignment_is_rejected():
    import pytest
    from embed_data_model import DatasetGraph, Patient, Exam
    graph = DatasetGraph()
    patient = graph.register(Patient("P"))
    exam = patient.add_exam(Exam("A"))
...
def test_source_field_updates_remove_old_aliases_and_pop_removes_current_ones():
    from embed_data_model import DatasetGraph, MammogramImage, RegionOfInterest
    graph = DatasetGraph()
